    ...
    <g:each status="i" in="${titles}" var="title">
        ...
        <div data-post-id="${i}"  class="emote-v2" id="emote-v2-${i}">
	        <div class="emote-v2-header clearfix">
				...
				<%--
					quick emote on top left doesn't work				
		<div id="qemote_${i}" class="quickEmote emote-v2-actions" >
			<button id="qemote_${i}" class="icon-edit emote-v2-action-button" onclick="javascript:javascript:quick_emote('${title.completeTitle}','${title.firstCategory}', 'emote-v2-${i}');_gaq.push(['_trackEvent', 'Quick Emotes', document.getElementById('category').options[document.getElementById('category').selectedIndex].value, 'Add', 1, false]);"></button>
		</div>
		--%>
            ...
            <div class="emote-v2-footer clearfix">
                <ul class="emote-v2-actions-left">
                    <li>
                        <button id="emote_camera_${i}" class="emote-v2-action-button" onclick="javascript:"><img src="/img/add_photo.png"/></button>
                    </li>
                </ul>
                <ul class="emote-v2-actions">
                     <li>
                         <button id="qemote_${i}" class="emote-v2-action-button" onclick="javascript:quick_emote('${title.completeTitle}','${title.firstCategory}', 'emote-v2-${i}');_gaq.push(['_trackEvent', 'Quick Emotes', document.getElementById('category').options[document.getElementById('category').selectedIndex].value, 'Add', 1, false]);">
                             <img src="/img/re-emote.png"/>
                         </button>
                     </li>

                    <g:if test="${UserDoing.isDoing(session.user.id,title.title)}">
                        <li>
                            <button id="fav_emote_${i}" class="emote-v2-action-button">
                                <img src="/img/doing_now_green.png"/>
                            </button>
                        </li>
                    </g:if>
                    <g:else>
                        <li>
                            <button id="fav_emote_${i}" class="emote-v2-action-button" onclick="javascript:doingNow(this.id,'${title.title}')">
                                <img src="/img/doing_now_grey.png"/>
                            </button>
                        </li>
                    </g:else>
                    ...
                    <li>
                        <button id="remote_${i}" class="emote-v2-action-button" onclick="javascript:flag_emote('${title.completeTitle}')">
                    ...
                    <g:if test="${favourites && favourites.contains(title.title)}">
                        <li>
                            <button id="fav_emote_${i}" class="emote-v2-action-button" onclick="javascript:removeFromFavourite(this.id,'${title.title}')">
                                <img src="/img/favourite_icon_on.png"/>
                            </button>
                        </li>
                    </g:if>
                    <g:else>
                        <li>
                            <button id="fav_emote_${i}" class="emote-v2-action-button" onclick="javascript:favouriteSubmit(this.id,'${title.title}')">
